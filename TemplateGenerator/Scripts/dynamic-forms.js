...
angular.module('app').directive('dynamicForm', ['$q', '$parse', '$http', '$templateCache', '$compile', '$document', '$timeout', function ( $q, $parse, $http, $templateCache, $compile, $document, $timeout) {
              ...
              //  Grab the template. either from the template attribute, or from the URL in templateUrl
                  ...
                  $http.get(attrs.templateUrl, { cache: $templateCache }).then(function (result) {
                    ...
                    bracket = function (model, base) {
                        props = model.split('.');
                        var abcd = (base || props.shift()) + (props.length ? "['" + props.join("']['") + "']" : '');
                    ...
                    buildFields = function (field, id) {
                        if (String(id).charAt(0) == '$') {
                            ...
                            if (angular.isDefined(supported[field.type].textBased) && supported[field.type].textBased) {
                                //newElement=newElement.wrap('<br><br>').parent();
                            ...
                            if (field.type === 'date') {
                                newElement.attr('id', 'datetype');
                                if (angular.isDefined(field.min)) { newElement.attr('min', field.min); }
                                if (angular.isDefined(field.max)) { newElement.attr('max', field.max); }
                                if (angular.isDefined(field.style)) { newElement.attr('style', field.style); }
                            }
